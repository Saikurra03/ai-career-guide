...
export default function ATSPage() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setOpenIndex(0);
      trackAnalysis("ats", "ATS Score Check");
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold mb-2">ATS Score Check</h1>
      <p className="text-text-muted mb-8">Paste a job description and your resume to see how well it passes Applicant Tracking Systems.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white border border-border rounded-2xl p-6">
          ...
          <p className="text-xs text-text-muted mb-3">Paste the full job posting you&apos;re applying for</p>
          <textarea
            value={jobDescription}
            onChange={(e) => setJobDescription(e.target.value)}
            rows={18}
            placeholder="Paste the job description here...&#10;&#10;Include the full posting with requirements, qualifications, and responsibilities for the best ATS analysis."
      ...
      <button
        ...
        className="w-full py-4 bg-primary text-white rounded-xl font-semibold text-base hover:bg-primary-hover transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-indigo-200 mb-6"
      >
        {analyzing ? "Analyzing ATS Match..." : "Check ATS Score"}
      </button>

      {(uploading || analyzing) && (
        <div className="flex items-center justify-center gap-3 py-3 bg-indigo-50 border border-indigo-200 rounded-xl text-sm text-primary font-medium mb-6">
          <div className="w-5 h-5 border-3 border-indigo-200 border-t-primary rounded-full animate-spin" />
          <span>{uploading ? "Extracting text..." : "AI is checking ATS compatibility..."}</span>
        </div>
      )}
      ...
      {sections.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold">ATS Analysis ({sections.length} sections)</h2>
      ...
      {!sections.length && !analyzing && !error && (
        <div className="text-center py-16 text-text-muted">
          ...
          <p>Paste your resume and a job description above, then click &quot;Check ATS Score&quot;.</p>
        </div>
      )}
    </div>
  );
