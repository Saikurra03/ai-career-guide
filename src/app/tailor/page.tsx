...
const colors = ["bg-violet-100 text-violet-700", "bg-emerald-100 text-emerald-700", "bg-sky-100 text-sky-700", "bg-rose-100 text-rose-700", "bg-amber-100 text-amber-700", "bg-teal-100 text-teal-700", "bg-indigo-100 text-indigo-700", "bg-pink-100 text-pink-700"];
...
export default function TailorPage() {
  ...
  const tailor = async () => {
    ...
    setTailoring(true); setError(""); setSections([]);
    try {
      ...
      const data = await res.json();
      if (data.error) { setError(data.error); setTailoring(false); return; }
      setRawResult(data.result);
      setSections(parseSections(data.result));
      trackAnalysis("tailor", "Tailor Resume");
      setOpenIndex(0);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold mb-2">Tailor Resume</h1>
      <p className="text-text-muted mb-8">Rewrite your resume to perfectly match a specific job description.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-6">
          <h2 className="font-bold mb-4">Target Job Description</h2>
          <p className="text-xs text-text-muted mb-3">Paste the job you want to tailor your resume for</p>
          <textarea
      ...
      <button
        onClick={tailor}
        disabled={tailoring || uploading}
        className="w-full py-4 bg-violet-500 text-white rounded-xl font-semibold text-base hover:bg-violet-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-violet-200 mb-6"
      >
        {tailoring ? "Tailoring Your Resume..." : "Tailor Resume for This Job"}
      </button>

      {(uploading || tailoring) && (
        <div className="flex items-center justify-center gap-3 py-3 bg-violet-50 border border-violet-200 rounded-xl text-sm text-violet-600 font-medium mb-6">
          <div className="w-5 h-5 border-3 border-violet-200 border-t-violet-500 rounded-full animate-spin" />
          <span>{uploading ? "Extracting text..." : "AI is tailoring your resume..."}</span>
        </div>
      )}
      ...
      {sections.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold">Tailored Results ({sections.length} sections)</h2>
      ...
      {!sections.length && !tailoring && !error && (
        <div className="text-center py-16 text-text-muted">
          <div className="text-5xl mb-4">✨</div>
          <p>Paste your resume and a target job description, then click &quot;Tailor Resume&quot;.</p>
        </div>
      )}
    </div>
  );
